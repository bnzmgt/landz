...
if (product.value) {
  const image =
    product.value.meta?.images?.[0] || product.value.image || "/default.jpg";

  const description =
    product.value.description?.slice(0, 150) ||
    `Dapatkan ${product.value.title} dengan harga terbaik dan kualitas terjamin.`;
    ...
    script: [
      {
        ...
        children: JSON.stringify({
          ...
          image: product.value.meta?.images?.length
            ? product.value.meta.images
            : [image],
...
<template>
  <div v-if="product" class="container mx-auto">
    <nav class="text-sm text-gray-400 mb-6 flex items-center gap-2">
      ...
      <NuxtLink to="/categories" class="hover:text-gray-900 transition"
        >Categories</NuxtLink
      >
    ...
    <div class="grid lg:grid-cols-2 gap-12 items-start">
      <div class="flex gap-4">
        <div class="flex flex-col items-center gap-2">
          ...
          <div
            ref="thumbContainer"
            class="flex flex-col gap-3 max-h-[420px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300"
          >
            <img
              v-for="img in galleryImages"
              :key="img"
              :src="img"
              class="w-20 h-20 object-cover border rounded cursor-pointer hover:border-primary"
              @click="mainImage = img"
            />
          </div>
        ...
        <div class="flex-1 rounded-xl flex items-center justify-center">
          <img
            v-if="mainImage"
            :src="mainImage"
            :alt="product.title"
      ...
      <div>
        ...
        <div class="flex items-center gap-3 mb-4">
          ...
          <span v-if="product.discount" class="badge badge-warning"
        ...
        <div class="mt-8">
          <div
            class="prose max-w-none transition-all"
