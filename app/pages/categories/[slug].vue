...
const { data: products } = await useAsyncData(`category-products-${slug}`, () =>
  queryCollection("products").where("category", "=", slug).all(),
);
...
if (category.value) {
  ...
  const description =
    ...
    `Temukan ${category.value.title} terbaik dengan harga murah, kualitas bagus, dan rating tinggi. Update terbaru 2026.`;
